            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                for (var j = 0; j < data[i].children.length; j++){
                    ...
                    if(data[i].children[j].target == "_blank"){
                        ulHtml += '<dd><a data-url="' + data[i].children[j].href + '" data-action="' + data[i].children[j].action+'" target="'+data[i].children[j].target+'">';
                    }else{
                        ulHtml += '<dd><a data-url="' + data[i].children[j].href + '" data-action="' + data[i].children[j].action +'">';
                    }
                ...
                if(data[i].target == "_blank"){
                    ulHtml += '<a data-url="' + data[i].href + '" data-action="' + data[i].action +'" target="'+data[i].target+'">';
                }else{
                    ulHtml += '<a data-url="' + data[i].href + '" data-action="' + data[i].action +'">';
                }
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ulHtml += '<i class="seraph '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
                ...
                element.tabAdd(tabFilter, {
			        title : title,
                    content: "<iframe src='"+_admin_+"/" + _this.attr("data-action") +"/"+_this.attr("data-url")+"' data-id='"+tabIdIndex+"'></frame>",
			        id : new Date().getTime()
			    })
				//当前窗口内容
				var curmenu = {
